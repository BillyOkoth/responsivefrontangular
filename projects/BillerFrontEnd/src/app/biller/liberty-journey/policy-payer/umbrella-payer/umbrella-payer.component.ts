...
@Component({
  selector: 'app-umbrella-payer',
  templateUrl: './umbrella-payer.component.html',
  styleUrls: ['./umbrella-payer.component.css']
})
export class UmbrellaPayerComponent implements OnInit {
    ...
    this.billerService.getMyPayers(payload).subscribe((resp: any) => {
      ...
      this.myPayers.forEach((value: any) => {
        value.policy_type.forEach((policy: any) => {
          // console.log(policy);
          if (policy.type.toLowerCase() === 'umbrella') {
            this.umbrella.push(value);
          }
        });
      });
