...
@Component({
  selector: 'app-upload-map-payer',
  templateUrl: './upload-map-payer.component.html',
  styleUrls: ['./upload-map-payer.component.css']
})
export class UploadMapPayerComponent implements OnInit {
  selected_file = [];
  fileList: any = [];
  uploading = false;
  fileName;
  ...
  getJson(value) {
    ...
    this.billerService.payer_insurance_columns = this.columns;

    sessionStorage.setItem('fetchedColumns', JSON.stringify(this.columns));
    // sessionStorage.setItem('',this.columns);
