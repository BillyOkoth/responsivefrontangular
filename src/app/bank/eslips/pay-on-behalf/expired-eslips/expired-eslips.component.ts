...
@Component({
  ...
  styleUrls: ['./expired-eslips.component.scss']
})
export class ExpiredEslipsComponent implements OnInit {
  ...
  downloadExcel() {
    ...
    this.excelDownload.buildExcelExpiredEslip('Expired Eslips', rows);
    this.loading = false;
  }
  ...
  getExpiredEslips() {
    ...
    this.accountService.getExpiredEslipbank(payload).subscribe(
      (response: any) => {
        this.loading = false;
        this.rows = response;
